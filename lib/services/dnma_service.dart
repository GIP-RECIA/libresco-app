...
class DnmaService {
  ...
  Future<int> sendMarkRequest(
    ...
    final cookieDnmaSession = createCookie(
      AppConfig().cookieDNMASessionName,
      Session().DNMASessionCookie,
      Account().domain,
      "/dnma",
    );
    final cookieDnmaId = createCookie(
      AppConfig().cookieDNMAIDName,
      Session().IDDNMACookie,
      Account().domain,
      "/dnma",
    );
    ...
    // Add body
    final body = {
      "date": (DateTime.now().millisecondsSinceEpoch / 1000).toInt().toString(),
  ...
  Future<bool> ensureLogin() async {
    if (Session().DNMASessionCookie == "") {
      ...
      final tgc = createCookie(
        AppConfig().casCookieName,
        Session().CASSessionCookie,
        AppConfig().casHost,
        "/cas",
      );
      ...
      try {
        while (!done && nbRequests < 10) {
          ...
          if (response.isRedirect) {
            ...
            // Set session cookies
            Session().setDNMASessionCookie(
              cookies.getValue(AppConfig().cookieDNMASessionName),
            );
            Session().setIDDNMACookie(
              cookies.getValue(AppConfig().cookieDNMAIDName),
            );
